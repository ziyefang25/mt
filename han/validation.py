...
class Validate:
    def __init__(self, config, model):
        ...
        self.dataset = TextDataset(config.validation_files, "validation_files", config.label_file, config.vocab_file, config.data_dir)
    ...
    def eval(self):
        ...
        with torch.no_grad():
            for (docs, labels, doc_lengths, sent_lengths) in self.dataloader:
                ...
                val_loss = self.criterion(scores.float(), labels.float()).mean()
            ...
            return metrics_results, val_loss
